...
import { Coffee, CheckCircle2, XCircle, Search, User, AlertTriangle } from 'lucide-react';
...
export default function SnacksPage() {
  ...
  const handleScan = useCallback(async (code: string) => {
    ...
    setResult(null);
    try {
      const res = await api.post<ScanResponse>('/operations/scan/snack-delivery', { qrCode: code.trim() });
      setResult(res);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Control de Refrigerios</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-secondary)' }}>Registrar entrega de refrigerio / snack</p>
        </div>
      ...
      {!searchMode && (
        <div className="rounded-2xl p-8 text-center" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)' }}>
          <Coffee className="w-12 h-12 mx-auto mb-3" style={{ color: '#ec4899' }} />
          <p className="text-white font-medium mb-4">Escanee QR para registrar entrega de refrigerio</p>
